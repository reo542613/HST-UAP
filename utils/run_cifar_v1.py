...
if __name__ == '__main__':
    ...
    trainset = torchvision.datasets.CIFAR10(root='./torch_datasets/cifar-data', train=True, download=True,
                                            transform=transforms_normalize)
    train_loader = torch.utils.data.DataLoader(trainset, batch_size=args.batch_size, shuffle=True, num_workers=2)

    testset = torchvision.datasets.CIFAR10(root='./torch_datasets/cifar-data', train=False, download=True,
                                           transform=transforms_normalize)
    ...
    for epoch in range(epochs):
        ...
        for i, (X, y) in enumerate(train_loader):
            ...
            flow_field = (flow_field * args.tau) / (L + 1e-6)  # 避免除 0
            ...
            adv_loss = F.cross_entropy(logits_adv, train_ori_logits.max(1)[1])
            adv_loss.backward()
            optimizer.step()
        ...
        logger.info('%d \t %.1f \t %.4f \t %.4f \t %.4f \t %.4f \t %.4f \t %.4f',
                    epoch, train_time - start_time, train_loss / train_n, L.data.cpu(),
                    train_ori_acc / (train_n + skipped), train_st_rate / train_n, train_noise_rate / train_n,
                    train_attack_rate / train_n)
    ...
    logger.info('%.4f \t %.4f \t %.2f \t %.4f \t %.4f \t %.4f \t %.4f', test_ori_acc / (test_n + skipped),
                np.mean(test_l2), test_time - start_time, test_adv_loss / test_n, test_st_rate / test_n,
                test_noise_rate / test_n, test_attack_rate / test_n)
