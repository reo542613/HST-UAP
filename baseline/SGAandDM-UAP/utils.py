...
def model_imgnet(model_name):
    ...
    # 1. 定义 Timm 模型映射 (将你的短名字映射到 timm 的标准库名字)
    timm_map = {
    ...
    # === A. 尝试加载 Timm 模型 (Transformers) ===
    if model_name in timm_map:
        try:
            # 使用 timm 工厂函数加载
            base_model = timm.create_model(timm_map[model_name], pretrained=True)
        except Exception as e:
            raise RuntimeError(f"Timm 加载失败 {model_name}: {e}")

    # === B. 尝试加载 Torchvision 模型 (CNNs) ===
    else:
        # 将输入转为小写 (例如 VGG19 -> vgg19) 以匹配 torchvision 函数名
        tv_name = model_name.lower()

        if hasattr(models, tv_name):
            try:
                # 获取函数对象
                model_fn = getattr(models, tv_name)
                # 尝试使用新版 API (weights='DEFAULT')
                base_model = model_fn(weights='DEFAULT')
            except TypeError:
                # 回退到旧版 API (pretrained=True)
                base_model = model_fn(pretrained=True)
        else:
            raise ValueError(f"不支持的模型名称: {model_name}. 请检查拼写或添加到映射表中。")

    # === C. 模型封装 (DataParallel + Normalizer) ===
    # 1. 放入 GPU 并行
    # 注意：建议先 .cuda() 再包 DataParallel，或者包完再 .cuda()
    base_model = base_model.cuda()
    base_model = nn.DataParallel(base_model)

    # 2. 创建归一化层
    normalize = Normalizer(mean=IMGNET_MEAN, std=IMGNET_STD).cuda()

    # 3. 组合最终模型：输入[0,1] -> Normalize -> (DataParallel -> GPUs) -> Output
    final_model = nn.Sequential(normalize, base_model)
    ...
    return final_model
from models import *
def model_cifar10(model_name):
    if model_name == 'VGG19':
        model = VGG('VGG19')
        model.load_state_dict(torch.load('./checkpoint/cifar10_vgg19.pth')['net'])
    elif model_name == 'ResNet56':
        model = torch.hub.load("chenyaofo/pytorch-cifar-models", "cifar10_resnet56", pretrained=True)
    elif model_name == 'DenseNet121':
        model = DenseNet121()
        model.load_state_dict(torch.load('./checkpoint/cifar10_dense121.pth')['net'])
    elif model_name == 'ResNet56_cifar100':
        model = torch.hub.load("chenyaofo/pytorch-cifar-models", "cifar100_resnet56", pretrained=True)
    elif model_name == 'VGG19_cifar100':
        ...
        assert 0
    model = model.cuda()
    normalize = Normalizer(mean=CIFAR_MEAN, std=CIFAR_STD).cuda()

    # 3. 组合最终模型：输入[0,1] -> Normalize -> (DataParallel -> GPUs) -> Output
    final_model = nn.Sequential(normalize, model)
...
def loader_imgnet(dir_data, nb_images = 50000, batch_size = 100, model_dimension = 256,center_crop=224):
    ...
    if nb_images < 50000:
        np.random.seed(0)
        sample_indices = range(0, 50000, 50)
...
def evaluate(model, loader, uap = None, n = 5,batch_size =None, DEVICE=None):
    ...
    if uap is not None:
        batch_size = batch_size
        if uap.dim() == 4:
            uap = uap.repeat([batch_size, 1, 1, 1]).to(DEVICE)
        else: 
